...
const Logo = ({ alwaysEnabled = false }: { alwaysEnabled?: boolean }) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const next = current + POINTS_PER_TICK;
      localStorage.setItem(LOGO_POINTS_KEY, String(next));
      window.dispatchEvent(
        new CustomEvent(LOGO_POINTS_EVENT, { detail: { points: next } }),
      );
    }, TICK_MS);
