...
import { getCartMeta, addCartMeta, removeCartMeta } from '@/lib/cartLocalStorage';
...
export const useCartItemQuantity = (productId: number) => {
  ...
  const item = cart?.goods?.find((g: CartGood) => g.nomenclature_id === productId);
...
export const useCartProducts = () => {
  ...
  const productQueries = useQueries({
    queries: cartItems.map((item: CartGood) => ({   // добавлен тип
  ...
  const itemsWithProducts = cartItems
    .map((item: CartGood, index: number) => {
    ...
    .filter((item: CartItemWithProduct) => item !== null);

  itemsWithProducts.sort((a: CartItemWithProduct, b: CartItemWithProduct) => (b.addedAt || 0) - (a.addedAt || 0));
...
export const useAddToCart = () => {
  ...
  return useMutation({
    mutationFn: ({ nomenclature_id, quantity }: { nomenclature_id: number; quantity?: number }) =>
